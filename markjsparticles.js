export class MarkJSParticles {
  ...
  drawEffects() {
    ...
    for (let i = this.effects.length - 1; i >= 0; i--) {
      ...
      for (let ii = effect.particles.length - 1; ii >= 0; ii--) {
        ...
        const effectGlowStrength = effect && effect.options && typeof effect.options.glowStrength !== 'undefined' ? Number(effect.options.glowStrength) : 0;

        if (effectGlowStrength > 0) {
          this.ctx.shadowColor = color;
          this.ctx.shadowBlur = Math.max(0, Math.round(size * effectGlowStrength));
  ...
  static Explosion = class {
    constructor(x, y, options) {
      ...
      for (let i = 0; i < this.particlesPerExplosion; i++) {
        this.particles.push(
          new MarkJSParticles.Particle(x, y, this.particlesMinSpeed, this.particlesMaxSpeed, this.particlesMinSize, this.particlesMaxSize, this.options)
        );
      }
  ...
  static Confetti = class {
    constructor(x, y, options) {
      ...
      this.updateParticle = function (particle, deltaTime, now) {
        ...
        particle.xv += Math.sin(t * (particle.swayFreq || 1) + (particle.swayPhase || 0)) * swayBase * (particle.sway || 1) * dtFactor;
        particle.angle = (particle.angle || 0) + (particle.angularVelocity || 0);
        particle.yv += particle.gravity;
        particle.xv *= particle.friction;
        particle.yv *= particle.friction;
        particle.x += particle.xv;
  ...
  static Particle = class {
    constructor(x, y, minSpeed, maxSpeed, minSize, maxSize, opts = {}) {
      ...
      this.friction = MarkJSParticles.randRange(o.frictionMin ?? 0.96, o.frictionMax ?? 0.995, false);
      ...
      const baseLifetimeMs = o.lifetimeMs != null ? Number(o.lifetimeMs) : MarkJSParticles.randRange(o.lifetimeMinMs, o.lifetimeMaxMs, false);
